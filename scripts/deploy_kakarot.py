...
ACCOUNT_REGISTRY_COMPILED = Path(
    BUILD_PATH, "account_registry.json").read_text("utf-8")
BLOCKHASH_REGISTRY_COMPILED = Path(
    BUILD_PATH, "blockhash_registry.json").read_text("utf-8")
...
async def main():
    ...
    logging.info("✅ Contract Account Class Hash: %s",
                 hex(evm_account_class_hash))
    ...
    kakarot_contract = Contract(
        address=contract_addresses[0], abi=kakarot_abi, client=client)
    ...
    with open('deployed_addresses.json', 'w') as file:
        json.dump(data, file)
    logging.info("✅ Account Registry Address: %s",
                 hex(account_registry_address))
    logging.info("✅ Blockhash Registry Address: %s",
                 hex(blockhash_registry_address))
